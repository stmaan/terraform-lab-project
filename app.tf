resource "kubernetes_deployment" "flask_app" {
  metadata {
    ...
    labels    = { app = "educational-app" }
  }

  spec {
    replicas = 2
    selector { match_labels = { app = "educational-app" } }
    template {
      metadata { labels = { app = "educational-app" } }
      spec {
        container {
	...
	env {
          ...
          port {
            container_port = 80 # <-- ИСПРАВЛЕНО (было 5000)
          }
        }
      }
    }
...
resource "kubernetes_service" "flask_service" {
  ...
  spec {
    ...
    port {
      port        = 80
      target_port = 80 # <-- ИСПРАВЛЕНО (было 5000)
      node_port   = 30085
    }
    type = "NodePort"
  }
